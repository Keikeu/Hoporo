...
export class Hero {
  ...
  get deployedCrystals() {
    return this._deployedCrystals;
  }
  get speed() {
    return this._speed;
  }
  ...
  set y(y) {
    ...
    this.crystals.forEach((c, i) => {
      if (!c.deployed) c.y = y + 0.5 + 0.25 * i;
    });
  }
  set z(z) {
    this._z = z;
  ...
  set deployedCrystals(deployedCrystals) {
    this._deployedCrystals = deployedCrystals;
  }
  set speed(speed) {
    this._speed = speed;
  }
  ...
  shoot(mM) {
    if (mM.isMoving) {
      switch (mM.direction) {
        case "up":
          this.missile.position.z = this.tween(
            this.missile.position.z,
            -1,
            mM.delta,
            6
          );
          break;
        case "right":
          this.missile.position.x = this.tween(
  ...
  grabCrystal(scene) {
    this.crystals.push(
      new Crystal(
        this.map,
        this.x,
        this.y + 0.5 + 0.25 * this.crystals.length,
        this.z
      )
    );
  ...
  deployCrystal(scene, gameState) {
    if (
      ...
      this.crystals[this.crystals.length - 1].deployed === false
    ) {
  ...
  isMovePossible(direction) {
    ...
    if (direction === "up") {
      if (this.isObstacle(x, z - 1)) {
        return false;
      }
    } else if (direction === "right") {
      if (this.isObstacle(x + 1, z)) {
        return false;
      }
...
function getMissileModel() {
  ...
  return new Promise((resolve) => {
    loader.load(
      "./media/models/missile/model.gltf",
      resolve,
      undefined,
      function (error) {
        console.error(error);
