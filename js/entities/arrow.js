  ...
  for (let i = 0; i < map.arrowsMap.length; i++) {
    for (let j = 0; j < map.arrowsMap[i].length; j++) {
      if (map.arrowsMap[i][j] !== 0) {
        if (map.arrowsMap[i][j] === 1)
          arrows.push(new Arrow(map, i, -0.5, j, "up"));
        else if (map.arrowsMap[i][j] === 2)
          arrows.push(new Arrow(map, i, -0.5, j, "right"));
        else if (map.arrowsMap[i][j] === 3)
          arrows.push(new Arrow(map, i, -0.5, j, "down"));
        else if (map.arrowsMap[i][j] === 4)
          arrows.push(new Arrow(map, i, -0.5, j, "left"));
