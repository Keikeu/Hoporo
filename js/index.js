...
function update() {
  ...
  if (!gameState.defeat && !gameState.win && heros[0]) {
    ...
    crystals.forEach((crystal) => {
      ...
      if (
        crystal.x === parseFloat(hero.x) &&
        crystal.z === parseFloat(hero.z) &&
        crystal.alive
      ) {
        crystal.die(scene);
    ...
    birds.forEach((bird) => {
      bird.move(delta);
      if (
        Math.abs(bird.x - hero.x) < 0.45 &&
        Math.abs(bird.z - hero.z) < 0.45 &&
        hero.y < 1
      ) {
        gameState.defeat = true;
        document
          .querySelector(".modal-container--defeat")
    ...
    spikes.forEach((spike) => {
      spike.move(delta);
      if (
        Math.abs(spike.x - hero.x) < 0.45 &&
        Math.abs(spike.z - hero.z) < 0.45 &&
        hero.y < 1
      ) {
        gameState.defeat = true;
        document
    ...
    slimes.forEach((slime) => {
      slime.move(delta);
      if (
        Math.abs(slime.x - hero.x) < 0.45 &&
        Math.abs(slime.z - hero.z) < 0.45 &&
        hero.y < 1
      ) {
        gameState.defeat = true;
        document
    ...
    if (
      heroMovement.isMoving ||
      heroMovement.shouldMove ||
      heroMovement.isDashing ||
      heroMovement.shouldDash
    ) {
    ...
    if (
      ...
      missile.position.x < gameState.map.m &&
      missile.position.z >= 0 &&
      missile.position.z < gameState.map.n
    ) {
      walls.forEach((wall) => {
        if (
          wall.x === parseFloat(missile.position.x) &&
          wall.z === parseFloat(missile.position.z)
        ) {
          hero.killMissile();
        }
      });
      gates.forEach((gate) => {
        if (
          gate.x === parseFloat(missile.position.x) &&
          gate.z === parseFloat(missile.position.z)
        ) {
          gate.die(scene);
      ...
      birds.forEach((bird) => {
        if (
          Math.abs(bird.x - parseFloat(missile.position.x)) < 0.4 &&
          Math.abs(bird.z - parseFloat(missile.position.z)) < 0.4
        ) {
          bird.die(scene);
      ...
      slimes.forEach((slime) => {
        if (
          Math.abs(slime.x - parseFloat(missile.position.x)) < 0.4 &&
          Math.abs(slime.z - parseFloat(missile.position.z)) < 0.4
        ) {
  ...
  if (gameState.win && hero.y < 11) {
    ...
    document
      .querySelector(".modal-container--next")
      .classList.remove("invisible");
    hero.y = -0.2;
  }
}
  ...
  if (!debouncePlay) {
    ...
    if (
      document
        ...
        .classList.contains("invisible") === false
    ) {
      initAudio();
      ...
      document
        .querySelector(".modal-container--win")
        .classList.contains("invisible") === false
    ) {
      gameState.level = 1;
      localStorage.setItem("level", gameState.level);
...
function getModel() {
  ...
  return new Promise((resolve) => {
    loader.load(
      "./media/models/cactus/model.gltf",
      resolve,
      undefined,
      function (error) {
        console.error(error);
